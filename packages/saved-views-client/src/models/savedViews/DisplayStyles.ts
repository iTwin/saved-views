...
/**
 * The JSON representation of the map imagery. Map imagery include the specification for the base layer (which was
 * originally represented by `BackgroundMapProps.providerName` && `BackgroundMapProps.providerData`) and additional map
 * layers. In earlier versions only a background map was supported as specified by the `providerName` and `mapType`
 * properties of `BackgroundMapSettings` object. In order to provide backward compatibility the original,
 * `BackgroundMapSettings` are synchronized with the `MapImagerySettings` base layer as long as the settings are
 * compatible. The non-base map layers may represent image layers generated by tile servers or model layers that are
...
/**
 * JSON representation of properties common to both {@linkcode ImageMapLayerProps} and {@linkcode ModelMapLayerProps}.
 */
...
/**
 * JSON representation of the settings associated with a map sublayer included within a MapLayerProps. A map sub layer
 * represents a set of objects within the layer that can be controlled separately. These are produced only from map
 * servers that produce images on demand and are not supported by tiled (cached) servers.
 */
...
export enum PlanarClipMaskMode {
  ...
  None = 0,
  /**
   * Mask based on priority. Different types of models have different default priorities as enumerated by
   * {@linkcode PlanarClipMaskPriority}. For example, background maps have the lowest priority, so they are masked by
   * all other types, while design models have the highest priority and are therefore never masked. The priority of a
   * reality model can be overridden by `PlanarClipMaskSettings.priority`. This is useful to allow one reality model to
   * mask another overlapping one.
   */
  Priority = 1,
  ...
  IncludeElements = 4,
  /**
   * Indicates that masks should be produced from the geometry of all GeometricElements in a view, **except** for a
   * specified set of excluded elements.
   */
  ExcludeElements = 5,
}
