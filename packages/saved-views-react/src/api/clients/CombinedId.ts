...
export class CombinedId implements ContextIds {
  ...
  constructor(source: number, resourceId: string, iTwinId: string, iModelId?: string);
  ...
  constructor(
    ...
    if (typeof discriminator === "number") {
      ...
      const combinedId = CombinedId.combine(source, resourceId ?? "", iTwinId ?? "", iModelId);
      if (combinedId) {
  ...
  public static combine(source: number, resourceId?: string, iTwinId?: string, iModelId?: string): string;
    ...
    try {
      ...
      ids.forEach((id, index) => {
        ...
        CombinedId.reorderByteGroupsToRFCinPlace(
          combinedBytes.subarray(1 + index * UUID_BYTE_COUNT, 1 + (index + 1) * UUID_BYTE_COUNT),
        );
      });
      return encode(combinedBytes).replace(/[=+/]/g, (c: string) => c === "=" ? "" : c === "+" ? "-" : "_");
    } catch (error) {
      return "";
    }
  }
  ...
  public static separate(combinedId = "") {
    const base64 = combinedId
      .replace(/[-_]/g, (c) => (c === "-" ? "+" : "/"))
      .padEnd(Math.ceil(combinedId.length / BASE64_BYTE_LENGTH) * BASE64_BYTE_LENGTH, "=");
    if (!isBase64(base64)) {
      return undefined;
    }
    ...
    for (let i = 0; i < idsBytes.byteLength / UUID_BYTE_COUNT; i++) {
      const id = new Uint8Array(idsBytes.slice(i * UUID_BYTE_COUNT, (i + 1) * UUID_BYTE_COUNT));
